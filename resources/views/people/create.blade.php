...
@section('content')
    <form method="POST" action="{{ route('people.store') }}">
        ...
        <div class="m-3">
            ...
            <input type="text" class="form-control @error('sa_id_num') is-invalid @enderror" name="sa_id_num" value="{{old('sa_id_num')}}">
            @error('sa_id_num')
                <div class="invalid-feedback"> {{ $message }} </div>
            @enderror
        </div>
        <div class="m-3">
            ...
            <input type="tel" class="form-control @error('mobile_number') is-invalid @enderror" name="mobile_number" value="{{old('mobile_number')}}">
            @error('mobile_number')
                <div class="invalid-feedback"> {{ $message }} </div>
            @enderror
        </div>
        ...
        <div class="m-3">
            ...
            <select class="form-select @error('language') is-invalid @enderror" name="language">
                ...
                @foreach ($languages as $language)
                    <option value="{{ $language->id }}" @if(old('language') == $language->id) selected @endif>{{ $language->label }}</option>
                @endforeach
            </select>
            @error('language')
                <div class="invalid-feedback"> {{ $message }} </div>
        ...
        <div class="m-3">
            ...
            <select class="form-select @error('interests') is-invalid @enderror" multiple data-bs-toggle="tooltip" data-bs-html="true" title="Select multiple Interest by holding <b>Ctrl</b>" name="interests[]">
                @foreach ($interests as $interest)
                    <option value="{{ $interest->id }}" @if(in_array($interest->id, old('interests') ?? [])) selected @endif>{{ $interest->label }}</option>
                @endforeach
            </select>
            @error('interests')
                <div class="invalid-feedback"> {{ $message }} </div>
